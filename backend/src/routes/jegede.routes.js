...
function fallbackBriefing(s) {
  const lines = [];
  // Diagnosis priority: trust > task disputes > money > funnel > growth
  let diagnosis = "";
  let nextMove = "";
  let risks = [];
  ...
  if (s.trust.pendingReports > 3) {
    ...
    diagnosis = `Escrow needs attention — ${s.gigs.disputedGigs} task disputes and ${s.services.disputedService} service disputes are open.`;
    nextMove = `Resolve disputes in /admin/disputes — release or refund quickly, auto-release pauses while disputed.`;
    risks.push(`Disputes: tasks ${s.gigs.disputedGigs}, services ${s.services.disputedService}`);
  } else if (s.gigs?.pendingWithdrawals > 5) {
    diagnosis = `Payouts are queuing — ${s.gigs.pendingWithdrawals} TrendTribe Wallet withdrawals are in review.`;
    ...
    diagnosis = `No paid tokens or task/service fees yet — marketplace is active but not monetized (admins are free, only non-admin fees count).`;
    nextMove = `Test a boost offer: 1 token for 24h Featured on a top item and see if sellers buy. Task 20% and service 20% fees only apply to non-admin.`;
    risks.push("Zero revenue from tokens + platform fees");
  } else {
    diagnosis = `Marketplace is healthy — ${s.totals.activeListings} active items (${s.books?.activeBookListings || 0} books), ${s.gigs?.openGigs || 0} open tasks, ${s.growth.newContactViews7d} contacts in 7 days.`;
    nextMove = `Double down on what works: boost the top-contacted item, keep ghost listings fresh, and watch TrendTribe Wallet tx ${s.gigs?.gigWalletTx7d || 0} in 7d.`;
  }
...
- Money: tokens ₦200 each via Flutterwave (hash-verified webhook), TrendTribe Wallet Naira top-up via Flutterwave, buy tokens with TrendTribe Wallet balance. Platform profit (PlatformProfit table) only counts non-admin: TOKEN_SOLD, GIG_CONFIRM_20, GIG_CANCEL_5, SERVICE_CONFIRM_20, GIG_TRANSFER_1P, GIG_WITHDRAW_1P. TrendTribe Wallet liability = sum gigBalance. Forgot-password uses Brevo and canonical https://trendtribe.app/reset-password?token=.

Data (all numbers are real DB counts, do not invent):
${JSON.stringify(s, null, 2)}

Task: When the admin says "update", explain in SIMPLE English:
- snapshot: 2 sentences, where we are now (users, active listings incl. books, tasks open/claimed/completed/disputed, 7d growth, TrendTribe Wallet tx)
- progress: funnel (favorites per listing, contact rate) and money (revenue non-admin, tokens, avg balance, platform fees by source, TrendTribe Wallet liability)
...
- risks: 1-3 short risks if any (cold listings, low token balance, high task liability, many open tasks, pending withdrawals/disputes)
