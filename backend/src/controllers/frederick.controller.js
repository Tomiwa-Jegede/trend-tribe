  ...
  try {
    ...
    try {
      ...
      if (fallbackProducts.length) {
        return res.status(200).json({
          reply: `I found ${fallbackProducts.length} item(s) for "${q}" — Jegede is briefly offline, Tomiwa did a quick search for you.`,
          products: fallbackProducts,
        });
      }
      return res.status(200).json({
        reply: `I couldn't find anything for "${q}" and Jegede is briefly offline. Tomiwa is here — try different words or check the Marketplace directly.`,
        products: [],
      });
    } catch (fallbackErr) {
      console.error("[FREDERICK FALLBACK ERROR]", fallbackErr.message);
      return res.status(200).json({
        reply: "Tomiwa here — Jegede is a bit busy right now (high demand). Please try again in a moment, or browse the Marketplace directly.",
        products: [],
      });
    }
  }
