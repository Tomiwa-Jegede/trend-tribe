...
| V2 | Student Seller | Create, edit, and remove listings with photos and accurate metadata | Creates listing at `/create-listing` with images + `Category`/`Condition`/`Subcategory` -> sees it on `/marketplace` and `/listings/:id`; edits at `/listings/:id/edit` or deletes own listing; manages at `/my-listings` (GHOST badge, Hide/Re-activate, Featured boost) | `POST /api/listings` with `Multer+Cloudinary` succeeds; detail shows gallery + `coverPosition`; `GET /api/listings/me` returns `_count.favorites`; Playwright create-listing + edit-listing + smoke `/my-listings` specs pass | DELIVERED | Milestone 2 + Milestone 5 (My Listings slice) | `Listing.images` + `imagePublicIds` stay in sync; unauthorized edit/delete returns 403; `Price` stored as Decimal(10,2) |
